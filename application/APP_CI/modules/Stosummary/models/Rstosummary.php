...
class Rstosummary extends CI_Model {
...
function load_default($start,$limit,$filter){
    ...
    function summary(){
        ...
        $this->db->select('SQL_CALC_FOUND_ROWS 
            FORMAT(hasilasset.total,0) AS total,
            FORMAT(hasilscan.scan,0) AS scan,
            FORMAT((hasilasset.total - hasilscan.scan),0) AS notscan', FALSE);
        $this->db->from('(
            ( SELECT COUNT(*) AS total FROM masset ) AS hasilasset,
            ( SELECT COUNT(*) AS scan FROM assetopname ) AS hasilscan
        )');
        $query = $this->db->get();
        $rows = $query->result_array();
